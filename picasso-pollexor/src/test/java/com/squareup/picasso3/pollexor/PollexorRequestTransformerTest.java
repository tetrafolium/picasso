...
public class PollexorRequestTransformerTest {
  ...
  private RequestTransformer transformer =
      new PollexorRequestTransformer(Thumbor.create(HOST));
  private RequestTransformer secureTransformer =
      new PollexorRequestTransformer(Thumbor.create(HOST, KEY));
  private RequestTransformer alwaysResizeTransformer =
      new PollexorRequestTransformer(Thumbor.create(HOST), true);
  ...
  public void simpleResize() {
    ...
    String expected =
        Thumbor.create(HOST).buildImage(IMAGE).resize(50, 50).toUrl();
    assertThat(output.uri.toString()).isEqualTo(expected);
  }
  ...
  public void simpleResizeWithCenterCrop() {
    Request input =
        new Request.Builder(IMAGE_URI).resize(50, 50).centerCrop().build();
    Request output = transformer.transformRequest(input);
    assertThat(output).isNotSameAs(input);
    assertThat(output.hasSize()).isFalse();
  ...
  public void simpleResizeWithCenterInside() {
    Request input =
        new Request.Builder(IMAGE_URI).resize(50, 50).centerInside().build();
    Request output = transformer.transformRequest(input);
    assertThat(output).isNotSameAs(input);
    assertThat(output.hasSize()).isFalse();
    ...
    String expected =
        Thumbor.create(HOST).buildImage(IMAGE).resize(50, 50).fitIn().toUrl();
    assertThat(output.uri.toString()).isEqualTo(expected);
  }
  ...
  public void simpleResizeWithEncryption() {
    ...
    String expected =
        Thumbor.create(HOST, KEY).buildImage(IMAGE).resize(50, 50).toUrl();
    assertThat(output.uri.toString()).isEqualTo(expected);
  }
