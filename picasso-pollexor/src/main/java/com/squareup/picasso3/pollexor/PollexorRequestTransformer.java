...
/**
 * A {@link RequestTransformer} that changes requests to use {@link Thumbor} for
 * some remote transformations. By default images are only transformed with
 * Thumbor if they have a size set.
 */
...
/**
 * Create a transformer for the specified {@link Thumbor} which always
 * transforms images using Thumbor even when resize is not set.
 */
public PollexorRequestTransformer(@NonNull Thumbor thumbor,
                                  boolean alwaysTransform) {
...
public Request transformRequest(@NonNull Request request) {
	...
	Uri uri = request.uri;
	if (uri == null) {
		throw new IllegalArgumentException("Null uri passed to " +
	...
	// Only transform requests that have resizes unless `alwaysTransform` is
	// set.
