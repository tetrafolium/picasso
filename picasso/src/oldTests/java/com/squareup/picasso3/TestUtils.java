...
class TestUtils {
  ...
  static final String STABLE_URI_KEY_1 =
      new Request.Builder(URI_1).stableKey(STABLE_1).build().key;
  static final File FILE_1 = new File("C:\\windows\\system32\\logo.exe");
  static final String FILE_KEY_1 =
      new Request.Builder(Uri.fromFile(FILE_1)).build().key;
  static final Uri FILE_1_URL = Uri.parse("file:///" + FILE_1.getPath());
  static final Uri FILE_1_URL_NO_AUTHORITY =
      Uri.parse("file:/" + FILE_1.getParent());
  static final Uri MEDIA_STORE_CONTENT_1_URL =
  ...
  static final String CONTENT_KEY_1 =
      new Request.Builder(CONTENT_1_URL).build().key;
  static final Uri CONTACT_URI_1 =
      CONTENT_URI.buildUpon().appendPath("1234").build();
  static final String CONTACT_KEY_1 =
      new Request.Builder(CONTACT_URI_1).build().key;
  static final Uri CONTACT_PHOTO_URI_1 = CONTENT_URI.buildUpon()
                                             .appendPath("1234")
                                             .appendPath(CONTENT_DIRECTORY)
                                             .build();
  static final String CONTACT_PHOTO_KEY_1 =
      new Request.Builder(CONTACT_PHOTO_URI_1).build().key;
  static final int RESOURCE_ID_1 = 1;
  static final String RESOURCE_ID_KEY_1 =
      new Request.Builder(RESOURCE_ID_1).build().key;
  static final Uri ASSET_URI_1 = Uri.parse("file:///android_asset/foo/bar.png");
  static final String ASSET_KEY_1 =
      new Request.Builder(ASSET_URI_1).build().key;
  ...
  static final Uri RESOURCE_ID_URI =
      new Uri.Builder()
          .scheme(SCHEME_ANDROID_RESOURCE)
          .authority(RESOURCE_PACKAGE)
          .appendPath(Integer.toString(RESOURCE_ID_1))
          .build();
  static final String RESOURCE_ID_URI_KEY =
      new Request.Builder(RESOURCE_ID_URI).build().key;
  static final Uri RESOURCE_TYPE_URI = new Uri.Builder()
                                           .scheme(SCHEME_ANDROID_RESOURCE)
                                           .authority(RESOURCE_PACKAGE)
  ...
  static final String CUSTOM_URI_KEY =
      new Request.Builder(CUSTOM_URI).build().key;
  ...
  static Resources mockResources(final String resValueString) {
    ...
    doAnswer(new Answer<Void>() {
      ...
      public Void answer(InvocationOnMock invocation) {
        ...
        .when(resources)
        .getValue(anyInt(), any(TypedValue.class), anyBoolean());
  ...
  static Action mockAction(String key, Uri uri, Object target, int resourceId,
    ...
    Request.Builder builder =
        new Request.Builder(uri, resourceId, DEFAULT_CONFIG).stableKey(key);
    if (priority != null) {
  ...
  static ImageView mockImageViewTarget() { return mock(ImageView.class); }

  static RemoteViews mockRemoteViews() { return mock(RemoteViews.class); }

  static Notification mockNotification() { return mock(Notification.class); }
    ...
    RequestHandler requestHandler = new RequestHandler() {
      ...
      public void load(@NonNull Picasso picasso, @NonNull Request request,
        ...
        RequestHandler.Result result =
            new RequestHandler.Result(defaultResult, MEMORY);
  ...
  static Picasso mockPicasso(RequestHandler requestHandler) {
    ...
    when(picasso.getRequestHandlers())
        .thenReturn(Collections.singletonList(requestHandler));
    return picasso;
  }
  ...
  static final Picasso.Listener NOOP_LISTENER = new Picasso.Listener() {
    @Override
    public void onImageLoadFailed(@NonNull Picasso picasso, @NonNull Uri uri,
                                  @NonNull Exception exception) {}
  };

  static final List<RequestTransformer> NO_TRANSFORMERS =
  ...
  static Picasso defaultPicasso(Context context, boolean hasRequestHandlers,
    ...
    return builder.callFactory(UNUSED_CALL_FACTORY)
  ...
  static final class EventRecorder implements EventListener {
    ...
    @Override
    public void bitmapDecoded(@NonNull Bitmap bitmap) {
      decodedBitmap = bitmap;
    }

    @Override
    public void bitmapTransformed(@NonNull Bitmap bitmap) {
