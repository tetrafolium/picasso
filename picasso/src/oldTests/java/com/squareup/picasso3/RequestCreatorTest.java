...
public void shutUp() {
	initMocks(this);
	when(picasso.transformRequest(any(Request.class)))
	.thenAnswer(TRANSFORM_REQUEST_ANSWER);
}
...
@Test
public void getReturnsNullIfNullUriAndResourceId()
...
public void intoTargetWithNullUriAndResourceIdSkipsAndCancels() {
	...
	new RequestCreator(picasso, null, 0)
...
public void targetActionWithDefaultTag() {
	new RequestCreator(picasso, URI_1, 0).into(mockTarget());
	verify(picasso).enqueueAndSubmit(actionCaptor.capture());
	assertThat(actionCaptor.getValue().getTag())
	.isEqualTo(actionCaptor.getValue());
}
...
public void intoImageViewWithQuickMemoryCacheCheckDoesNotSubmit() {
	...
	Picasso picasso = spy(
		new Picasso(RuntimeEnvironment.application, mock(Dispatcher.class),
		            UNUSED_CALL_FACTORY, null, cache, null, NO_TRANSFORMERS,
...
public void intoImageViewSetsPlaceholderWithResourceId() {
	...
	.into(target);
	ArgumentCaptor<Drawable> drawableCaptor =
...
@Test
public void
intoImageViewWithFitAndNoDimensionsQueuesDeferredImageViewRequest() {
...
public void intoImageViewWithFitAndResizeThrows() {
	try {
		...
		fail(
			"Calling into() ImageView with fit() and resize() should throw exception");
	} catch (IllegalStateException ignored) {
	}
}
...
public void intoRemoteViewsNotificationWithNullRemoteViewsThrows() {
	try {
		new RequestCreator(picasso, URI_1, 0)
		.into(null, 0, 0, mockNotification());
...
public void intoRemoteViewsNotificationWithNullNotificationThrows() {
	try {
		new RequestCreator(picasso, URI_1, 0)
		.into(mockRemoteViews(), 0, 0, (Notification)null);
...
public void intoRemoteViewsWidgetWithFitThrows() {
	try {
		...
		new RequestCreator(picasso, URI_1, 0)
		.fit()
		.into(remoteViews, 1, new int[] {1, 2, 3});
...
public void intoRemoteViewsNotificationWithFitThrows() {
	try {
		...
		new RequestCreator(picasso, URI_1, 0)
		.fit()
		.into(remoteViews, 1, 1, mockNotification());
		fail("Calling fit() into remote views should throw exception");
	} catch (IllegalStateException ignored) {
	}
}
