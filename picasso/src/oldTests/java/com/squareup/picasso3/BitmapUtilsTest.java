...
public void calculateInSampleSizeResizeCenterInside() {
	final BitmapFactory.Options options = new BitmapFactory.Options();
	Request data =
		new Request.Builder(URI_1).centerInside().resize(100, 100).build();
	calculateInSampleSize(data.targetWidth, data.targetHeight, 400, 200,
	                      options, data);
...
public void calculateInSampleSizeKeepAspectRatioWithWidth() {
	...
	calculateInSampleSize(data.targetWidth, data.targetHeight, 800, 200,
	                      options, data);
	assertThat(options.inSampleSize).isEqualTo(2);
}
...
public void inJustDecodeBoundsIfResizing() {
	...
	final Request requiresResize =
		new Request.Builder(URI_1).resize(20, 15).build();
	final BitmapFactory.Options resizeOptions =
		createBitmapOptions(requiresResize);
...
public void createWithConfigAndNotInJustDecodeBoundsOrInPurgeable() {
	// Given a config, must return bitmap options and false
	// inJustDecodeBounds/inPurgeable
