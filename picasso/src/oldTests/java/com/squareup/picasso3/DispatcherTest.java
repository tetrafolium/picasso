...
public class DispatcherTest {
  ...
  public void performSubmitWithShutdownAttachesRequest() {
    BitmapHunter hunter = mockHunter(
        URI_KEY_1 + KEY_SEPARATOR, new RequestHandler.Result(bitmap1, MEMORY));
  ...
  public void performSubmitWithFetchAction() {
    ...
    FetchAction fetchAction1 = new FetchAction(
        mockPicasso(), new Request.Builder(URI_1).tag(pausedTag).build(), null);
    FetchAction fetchAction2 = new FetchAction(
        mockPicasso(), new Request.Builder(URI_1).tag(pausedTag).build(), null);
  ...
  public void performCancelWithFetchActionWithCallback() {
    ...
    FetchAction fetchAction1 = new FetchAction(
        mockPicasso(), new Request.Builder(URI_1).tag(pausedTag).build(),
        callback);
  ...
  public void performCancelUnqueuesAndDetachesPausedRequest() {
    ...
    BitmapHunter hunter =
        mockHunter(URI_KEY_1 + KEY_SEPARATOR,
                   new RequestHandler.Result(bitmap1, MEMORY), action);
    dispatcher.hunterMap.put(URI_KEY_1 + KEY_SEPARATOR, hunter);
  ...
  public void performCompleteWithNoStoreMemoryPolicy() {
    Request data =
        new Request.Builder(URI_1).memoryPolicy(MemoryPolicy.NO_STORE).build();
    Action action = noopAction(data);
    BitmapHunter hunter = new BitmapHunter(mockPicasso(), dispatcher, cache,
                                           action, EMPTY_REQUEST_HANDLER);
    hunter.result = new RequestHandler.Result(bitmap1, MEMORY);
  ...
  public void performErrorCleansUpAndDoesNotPostToMainIfCancelled() {
    BitmapHunter hunter =
        mockHunter(URI_KEY_1, new RequestHandler.Result(bitmap1, MEMORY));
  ...
  public void performRetrySkipsIfHunterIsCancelled() {
    BitmapHunter hunter =
        mockHunter(URI_KEY_2, new RequestHandler.Result(bitmap1, MEMORY));
    when(hunter.isCancelled()).thenReturn(true);
  ...
  public void performRetryForContentLengthResetsNetworkPolicy() {
    ...
    BitmapHunter hunter = new BitmapHunter(mockPicasso(), dispatcher, cache,
                                           mockAction(URI_KEY_2, URI_2),
                                           RETRYING_REQUEST_HANDLER);
  ...
  public void performRetryDoesNotMarkForReplayIfNotSupported() {
    ...
    when(hunter.shouldRetry(anyBoolean(), any(NetworkInfo.class)))
        .thenReturn(false);
    when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
    dispatcher.performRetry(hunter);
    assertThat(dispatcher.hunterMap).isEmpty();
  ...
  @Test
  public void
  performRetryMarksForReplayIfSupportedScansNetworkChangesAndShouldNotRetry() {
    NetworkInfo networkInfo = mockNetworkInfo(true);
    Action action = mockAction(URI_KEY_1, URI_1, mockTarget());
    BitmapHunter hunter = mockHunter(
        URI_KEY_1, new RequestHandler.Result(bitmap1, MEMORY), action);
    when(hunter.supportsReplay()).thenReturn(true);
    when(hunter.shouldRetry(anyBoolean(), any(NetworkInfo.class)))
        .thenReturn(false);
    when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
  ...
  public void performRetryRetriesIfNoNetworkScanning() {
    BitmapHunter hunter =
        mockHunter(URI_KEY_1, new RequestHandler.Result(bitmap1, MEMORY),
                   mockAction(URI_KEY_1, URI_1));
    when(hunter.shouldRetry(anyBoolean(), isNull(NetworkInfo.class)))
        .thenReturn(true);
    Dispatcher dispatcher = createDispatcher(false);
    dispatcher.performRetry(hunter);
    assertThat(dispatcher.hunterMap).isEmpty();
  ...
  @Test
  public void
  performNetworkStateChangeWithConnectedInfoDifferentInstanceIgnores() {
    Dispatcher dispatcher = createDispatcher(serviceMock);
  ...
  public void nullIntentOnReceiveDoesNothing() {
    ...
    NetworkBroadcastReceiver receiver =
        new NetworkBroadcastReceiver(dispatcher);
  ...
  public void nullExtrasOnReceiveConnectivityAreOk() {
    ...
    when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
    when(context.getSystemService(CONNECTIVITY_SERVICE))
  ...
  private Dispatcher createDispatcher(ExecutorService service,
    ...
    when(connectivityManager.getActiveNetworkInfo())
        .thenReturn(scansNetworkChanges ? mock(NetworkInfo.class) : null);
    when(context.getSystemService(Context.CONNECTIVITY_SERVICE))
        .thenReturn(connectivityManager);
    when(context.checkCallingOrSelfPermission(anyString()))
        .thenReturn(scansNetworkChanges ? PERMISSION_GRANTED
                                        : PERMISSION_DENIED);
    return new Dispatcher(context, service, new Handler(Looper.getMainLooper()),
                          cache, stats);
  }

  private static final RequestHandler RETRYING_REQUEST_HANDLER =
      new RequestHandler() {
        @Override
  ...
  private static final RequestHandler EMPTY_REQUEST_HANDLER =
      new RequestHandler() {
        @Override
        public boolean canHandleRequest(@NonNull Request data) {
          return false;
