...
public class NetworkRequestHandlerTest {
  ...
  @Test
  public void doesNotForceLocalCacheOnlyWithAirplaneModeOffAndRetryCount()
      throws Exception {
  ...
  public void unknownContentLengthFromDiskThrows() throws Exception {
    ...
    responses.add(
        responseOf(body).newBuilder().cacheResponse(responseOf(null)).build());
    Action action = TestUtils.mockAction(URI_KEY_1, URI_1);
    final CountDownLatch latch = new CountDownLatch(1);
    networkHandler.load(picasso, action.request, new RequestHandler.Callback() {
      @Override
  ...
  public void shouldHandleSchemeInsensitiveCase() {
    ...
    for (String scheme : schemes) {
      ...
      assertThat(networkHandler.canHandleRequest(TestUtils.mockRequest(uri)))
          .isTrue();
    }
  }
  ...
  private static Response responseOf(ResponseBody body) {
    return new Response.Builder()
        ...
        .protocol(HTTP_1_1)
        .request(
            new okhttp3.Request.Builder().url("http://example.com").build())
        .message("OK")
        .body(body)
        .build();
  }
