...
public final class PicassoTest {
  ...
  public void setUp() {
    initMocks(this);
    picasso = new Picasso(context, dispatcher, UNUSED_CALL_FACTORY, null, cache,
                          listener, NO_TRANSFORMERS, NO_HANDLERS,
                          Collections.singletonList(eventRecorder), ARGB_8888,
                          false, false);
  }
  ...
  public void completeInvokesSuccessOnAllSuccessfulRequests() {
    ...
    BitmapHunter hunter =
        mockHunter(URI_KEY_1, new RequestHandler.Result(bitmap, MEMORY));
  ...
  public void throwWhenTransformRequestReturnsNull() {
    ...
    Picasso picasso =
        new Picasso(context, dispatcher, UNUSED_CALL_FACTORY, null, cache,
                    listener, Collections.singletonList(brokenTransformer),
  ...
  public void builderWithoutRequestHandler() {
    Picasso picasso =
        new Picasso.Builder(RuntimeEnvironment.application).build();
  ...
  public void invalidateString() {
    Request request =
        new Request.Builder(Uri.parse("https://example.com")).build();
  ...
  public void invalidateFile() {
    Request request =
        new Request.Builder(Uri.fromFile(new File("/foo/bar/baz"))).build();
    cache.set(request.key, makeBitmap(1, 1));
    assertThat(cache.size()).isEqualTo(1);
  ...
  public void clonedRequestHandlersAreIndependent() {
    Picasso original =
        defaultPicasso(RuntimeEnvironment.application, false, false);
  ...
  public void cloneSharesStatefulInstances() {
    ...
    for (int i = 0, n = child.requestHandlers.size(); i < n; i++) {
      assertThat(child.requestHandlers.get(i))
          .isInstanceOf(parent.requestHandlers.get(i).getClass());
    }
