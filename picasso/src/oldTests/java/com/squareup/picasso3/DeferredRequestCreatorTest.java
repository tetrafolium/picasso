...
public void cancelClearsCallback() {
	...
	DeferredRequestCreator request = new DeferredRequestCreator(
		mock(RequestCreator.class), target, callback);
...
public void cancelClearsTag() {
	...
	DeferredRequestCreator request =
		new DeferredRequestCreator(creator, target, null);
	request.cancel();
...
public void waitsForAnotherLayoutIfWidthOrHeightIsZero() {
	...
	DeferredRequestCreator request =
		new DeferredRequestCreator(creator, target, null);
	request.onPreDraw();
	verify(target.getViewTreeObserver(), never())
	.removeOnPreDrawListener(request);
	verifyZeroInteractions(creator);
}
