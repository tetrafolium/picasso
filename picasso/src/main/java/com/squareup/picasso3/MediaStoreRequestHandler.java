...
class MediaStoreRequestHandler extends ContentStreamRequestHandler {
  private static final String[] CONTENT_ORIENTATION =
      new String[] {Images.ImageColumns.ORIENTATION};

  MediaStoreRequestHandler(Context context) { super(context); }
    ...
    try {
      ...
      if (request.hasSize()) {
        PicassoKind picassoKind =
            getPicassoKind(request.targetWidth, request.targetHeight);
        ...
        if (isVideo) {
          // Since MediaStore doesn't provide the full screen kind thumbnail, we
          // use the mini kind instead which is the largest thumbnail size can
          // be fetched from MediaStore.
          int kind = (picassoKind == FULL) ? Video.Thumbnails.MINI_KIND
  ...
  protected int getExifOrientation(Uri uri) {
    ...
    try {
      ContentResolver contentResolver = context.getContentResolver();
      cursor =
          contentResolver.query(uri, CONTENT_ORIENTATION, null, null, null);
