...
final class BitmapUtils {
  ...
  /**
   * Decode a byte stream into a Bitmap. This method will take into account
   * additional information about the supplied request in order to do the
   * decoding efficiently (such as through leveraging
   * {@code inSampleSize}).
   */
  static Bitmap decodeStream(Source source, Request request)
      throws IOException {
    ExceptionCatchingSource exceptionCatchingSource =
        new ExceptionCatchingSource(source);
    BufferedSource bufferedSource = Okio.buffer(exceptionCatchingSource);
    Bitmap bitmap = SDK_INT >= 28 ? decodeStreamP(request, bufferedSource)
  ...
  private static Bitmap decodeStreamP(Request request,
                                      BufferedSource bufferedSource)
      throws IOException {
    ImageDecoder.Source imageSource = ImageDecoder.createSource(
        ByteBuffer.wrap(bufferedSource.readByteArray()));
    return decodeImageSource(imageSource, request);
  }

  private static Bitmap decodeStreamPreP(Request request,
                                         BufferedSource bufferedSource)
      throws IOException {
    boolean isWebPFile = Utils.isWebPFile(bufferedSource);
    boolean isPurgeable =
        request.purgeable && SDK_INT < Build.VERSION_CODES.LOLLIPOP;
    BitmapFactory.Options options = createBitmapOptions(request);
    boolean calculateSize = requiresInSampleSize(options);

    Bitmap bitmap;
    // We decode from a byte array because, a) when decoding a WebP network
    // stream, BitmapFactory throws a JNI Exception, so we workaround by
    // decoding a byte array, or b) user requested purgeable, which only affects
    // bitmaps decoded from byte arrays.
    if (isWebPFile || isPurgeable) {
      ...
      if (calculateSize) {
        BitmapFactory.decodeStream(bufferedSource.peek().inputStream(), null,
                                   options);
        calculateInSampleSize(request.targetWidth, request.targetHeight,
                              checkNotNull(options, "options == null"),
                              request);
      }
      bitmap = BitmapFactory.decodeStream(bufferedSource.inputStream(), null,
                                          options);
    }
    if (bitmap == null) {
  ...
  @RequiresApi(28)
  private static Bitmap decodeResourceP(Context context, final Request request)
      throws IOException {
  ...
  private static Bitmap decodeResourcePreP(Resources resources, int id,
                                           Request request) {
  ...
  @RequiresApi(28)
  private static Bitmap decodeImageSource(ImageDecoder.Source imageSource,
                                          final Request request)
      throws IOException {
    return ImageDecoder.decodeBitmap(
        imageSource, new ImageDecoder.OnHeaderDecodedListener() {
          @Override
          public void onHeaderDecoded(@NonNull ImageDecoder imageDecoder,
            ...
            if (request.hasSize()) {
              ...
              if (shouldResize(request.onlyScaleDown, size.getWidth(),
                               size.getHeight(), request.targetWidth,
                               request.targetHeight)) {
                imageDecoder.setTargetSize(request.targetWidth,
                                           request.targetHeight);
              }
            }
          }
  ...
  static boolean isXmlResource(Resources resources,
  ...
  static final class ExceptionCatchingSource extends ForwardingSource {
    ...
    ExceptionCatchingSource(Source delegate) { super(delegate); }
    ...
    void throwIfCaught() throws IOException {
      if (thrownException != null) {
        // TODO: Log when Android returns a non-null Bitmap after swallowing an
        // IOException.
  ...
  private BitmapUtils() { throw new AssertionError("No instances."); }
}
