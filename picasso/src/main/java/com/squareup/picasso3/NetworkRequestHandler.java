...
final class NetworkRequestHandler extends RequestHandler {
  ...
  @Override
  public void load(@NonNull final Picasso picasso,
    ...
    callFactory.newCall(callRequest).enqueue(new okhttp3.Callback() {
      ...
      public void onResponse(Call call, Response response) {
        if (!response.isSuccessful()) {
          callback.onError(
              new ResponseException(response.code(), request.networkPolicy));
          return;
        }

        // Cache response is only null when the response comes fully from the
        // network. Both completely cached and conditionally cached responses
        // will have a non-null cache response.
        Picasso.LoadedFrom loadedFrom =
            response.cacheResponse() == null ? NETWORK : DISK;

        // Sometimes response content length is zero when requests are being
        // replayed. Haven't found root cause to this but retrying the request
        // seems safe to do so.
        ResponseBody body = response.body();
        if (loadedFrom == DISK && body.contentLength() == 0) {
          body.close();
          callback.onError(new ContentLengthException(
  ...
  private static okhttp3.Request createRequest(Request request) {
    ...
    okhttp3.Request.Builder builder =
        new okhttp3.Request.Builder().url(uri.toString());
  ...
  static class ContentLengthException extends RuntimeException {
    ContentLengthException(String message) { super(message); }
  }
