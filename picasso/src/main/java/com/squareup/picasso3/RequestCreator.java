...
public class RequestCreator {
   ...
   * target
   * {@link ImageView} in order to ensure behavior in situations where views are
   * recycled. This method will prevent that behavior and retain any already set
   * image.
   */
  @NonNull
   ...
   * requested image is not immediately available in the memory cache then this
   * resource will be set on the target
   * {@link ImageView}.
   */
  @NonNull
  ...
  /**
   * Attempt to resize the image to fit exactly into the target {@link
   * ImageView}'s bounds. This will result in delayed execution of the request
   * until the {@link ImageView} has been laid out. <p> <em>Note:</em> This
   * method works only when your target is an {@link ImageView}.
   */
  @NonNull
   ...
   * rather than distorting the aspect ratio. This cropping technique scales the
   * image so that it fills the requested bounds and then crops the extra.
   */
  @NonNull
   ...
   * rather than distorting the aspect ratio. This cropping technique scales the
   * image so that it fills the requested bounds and then crops the extra,
   * preferring the contents at {@code alignGravity}.
   */
  @NonNull
  ...
  /**
   * Centers an image inside of the bounds specified by {@link #resize(int,
   * int)}. This scales the image so that both dimensions are equal to or less
   * than the requested bounds.
   */
  @NonNull
   ...
   * {@link BitmapFactory.Options#inPreferredConfig its documentation} for more
   * details.
   */
  @NonNull
   ...
   * priority, except for
   * {@link #fetch()} requests, which have {@link Priority#LOW} priority by
   * default.
   */
  @NonNull
  ...
  transform(@NonNull List<? extends Transformation> transformations) {
   ...
   * additional policy options using the varargs parameter.
   */
  @NonNull
  ...
  /**
   * Set inPurgeable and inInputShareable when decoding. This will force the
   * bitmap to be decoded from a byte array instead of a stream, since
   * inPurgeable only affects the former. <p> <em>Note</em>: as of API level 21
   * (Lollipop), the inPurgeable field is deprecated and will be ignored.
   */
  @NonNull
  ...
  /**
   * Asynchronously fulfills the request without a {@link ImageView} or {@link
   * BitmapTarget}. This is useful when you want to warm up the cache with an
   * image. <p> <em>Note:</em> It is safe to invoke this method from any thread.
   */
  public void fetch() { fetch(null); }

  /**
   * Asynchronously fulfills the request without a {@link ImageView} or {@link
   * BitmapTarget}, and invokes the target {@link Callback} with the result.
   * This is useful when you want to warm up the cache with an image. <p>
   * <em>Note:</em> The {@link Callback} param is a strong reference and will
   * prevent your
   * {@link android.app.Activity} or {@link android.app.Fragment} from being
   * garbage collected until the request is completed.
   */
      ...
      if (shouldReadFromMemoryCache(request.memoryPolicy)) {
        ...
        if (bitmap != null) {
          if (picasso.loggingEnabled) {
            log(OWNER_MAIN, VERB_COMPLETED, request.plainId(),
                "from " + MEMORY);
          }
          if (callback != null) {
            callback.onSuccess();
  ...
  /**
   * Asynchronously fulfills the request into the specified {@link
   * BitmapTarget}. In most cases, you should use this when you are dealing with
   * a custom {@link android.view.View View} or view holder which should
   * implement the {@link BitmapTarget} interface. <p> Implementing on a {@link
   * android.view.View View}: <blockquote><pre> public class ProfileView extends
   * FrameLayout implements Target {
   *   {@literal @}Override public void onBitmapLoaded(Bitmap bitmap, LoadedFrom
   * from) { setBackgroundDrawable(new BitmapDrawable(bitmap));
   *   }
   *
   *   {@literal @}Override public void onBitmapFailed(Exception e, Drawable
   * errorDrawable) { setBackgroundDrawable(errorDrawable);
   *   }
   *
   *   {@literal @}Override public void onPrepareLoad(Drawable
   * placeHolderDrawable) { setBackgroundDrawable(placeHolderDrawable);
   *   }
   * }
   * </pre></blockquote>
   ...
   *
   *   {@literal @}Override public void onBitmapLoaded(Bitmap bitmap, LoadedFrom
   * from) { frame.setBackgroundDrawable(new BitmapDrawable(bitmap));
   *   }
   *
   *   {@literal @}Override public void onBitmapFailed(Exception e, Drawable
   * errorDrawable) { frame.setBackgroundDrawable(errorDrawable);
   *   }
   *
   *   {@literal @}Override public void onPrepareLoad(Drawable
   * placeHolderDrawable) { frame.setBackgroundDrawable(placeHolderDrawable);
   *   }
   * }
   * </pre></blockquote>
   * <p>
  ...
  public void into(@NonNull BitmapTarget target) {
    ...
    Action action = new BitmapTargetAction(picasso, target, request,
                                           errorDrawable, errorResId);
    picasso.enqueueAndSubmit(action);
  }

  /**
   * Asynchronously fulfills the request into the specified {@link RemoteViews}
   * object with the given {@code viewId}. This is used for loading bitmaps into
   * a {@link Notification}.
   */
  ...
  public void into(@NonNull RemoteViews remoteViews, @IdRes int viewId,
    ...
    Request request = createRequest(started);
    RemoteViewsAction action = new NotificationAction(
        picasso, request, errorResId,
        new RemoteViewsTarget(remoteViews, viewId), notificationId,
        notification, notificationTag, callback);
   ...
   */
  public void into(@NonNull RemoteViews remoteViews, @IdRes int viewId,
                   int appWidgetId, @Nullable Callback callback) {
   ...
   */
  public void into(@NonNull RemoteViews remoteViews, @IdRes int viewId,
                   @NonNull int[] appWidgetIds, @Nullable Callback callback) {
    long started = System.nanoTime();
    ...
    Request request = createRequest(started);
    RemoteViewsAction action = new AppWidgetAction(
        picasso, request, errorResId,
   ...
   */
  public void into(@NonNull ImageView target) { into(target, null); }

  /**
   * Asynchronously fulfills the request into the specified {@link ImageView}
   * and invokes the target {@link Callback} if it's not {@code null}. <p>
   * <em>Note:</em> The {@link Callback} param is a strong reference and will
   * prevent your
   * {@link android.app.Activity} or {@link android.app.Fragment} from being
   * garbage collected. If you use this method, it is <b>strongly</b>
  ...
  public void into(@NonNull ImageView target, @Nullable Callback callback) {
    ...
    if (shouldReadFromMemoryCache(request.memoryPolicy)) {
      ...
      if (bitmap != null) {
        ...
        setResult(target, picasso.context, result, noFade,
    ...
    Action action = new ImageViewAction(picasso, target, request, errorDrawable,
                                        errorResId, noFade, callback);
    picasso.enqueueAndSubmit(action);
  }
  ...
  private Request createRequest(long started) {
    ...
    if (transformed != request) {
      // If the request was changed, copy over the id and timestamp from the
      // original.
      transformed.id = id;
      transformed.started = started;

      if (loggingEnabled) {
        log(OWNER_MAIN, VERB_CHANGED, transformed.logId(),
            "into " + transformed);
      }
    }
  ...
  private void performRemoteViewInto(Request request,
