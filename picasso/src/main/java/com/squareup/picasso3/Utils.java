...
final class Utils {
  ...
  */
  ...
  private static final ByteString WEBP_FILE_HEADER_WEBP =
      ByteString.encodeUtf8("WEBP");

  private Utils() {
    // No instances.
  }

  public static @NonNull<T> T checkNotNull(@Nullable T value,
  ...
  static File createDefaultCacheDir(Context context) {
    File cache =
        new File(context.getApplicationContext().getCacheDir(), PICASSO_CACHE);
    if (!cache.exists()) {
  ...
  static long calculateDiskCacheSize(File dir) {
    ...
    try {
      ...
      long blockCount = SDK_INT < JELLY_BEAN_MR2 ? (long)statFs.getBlockCount()
                                                 : statFs.getBlockCountLong();
      // noinspection deprecation
      long blockSize = SDK_INT < JELLY_BEAN_MR2 ? (long)statFs.getBlockSize()
  ...
  static int calculateMemoryCacheSize(Context context) {
    ...
    boolean largeHeap =
        (context.getApplicationInfo().flags & FLAG_LARGE_HEAP) != 0;
    int memoryClass =
        largeHeap ? am.getLargeMemoryClass() : am.getMemoryClass();
  ...
  static boolean isAirplaneModeOn(Context context) {
    ...
    try {
      ...
      // https://github.com/square/picasso/issues/761, some devices might crash
      // here, assume that airplane mode is off.
      return false;
  ...
  private static class PicassoThread extends Thread {
    PicassoThread(Runnable r) { super(r); }
