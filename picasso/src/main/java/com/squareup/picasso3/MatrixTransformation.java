...
final class MatrixTransformation implements Transformation {
  ...
  MatrixTransformation(Request data) { this.data = data; }
  ...
  static Bitmap transformResult(Request data, Bitmap result,
    ...
    if (data.needsMatrixTransform() || exifOrientation != 0) {
      ...
      // EXIf interpretation should be done before cropping in case the
      // dimensions need to be recalculated
      ...
      if (data.centerCrop) {
        ...
        float widthRatio = targetWidth != 0 ? targetWidth / (float)inWidth
                                            : targetHeight / (float)inHeight;
        float heightRatio = targetHeight != 0 ? targetHeight / (float)inHeight
        ...
        if (widthRatio > heightRatio) {
          ...
          if ((data.centerCropGravity & Gravity.TOP) == Gravity.TOP) {
            drawY = 0;
          } else if ((data.centerCropGravity & Gravity.BOTTOM) ==
                     Gravity.BOTTOM) {
          ...
          if ((data.centerCropGravity & Gravity.LEFT) == Gravity.LEFT) {
            drawX = 0;
          } else if ((data.centerCropGravity & Gravity.RIGHT) ==
                     Gravity.RIGHT) {
        ...
        if (shouldResize(onlyScaleDown, inWidth, inHeight, targetWidth,
                         targetHeight)) {
        ...
        // If an explicit target size has been specified and they do not match
        // the results bounds, pre-scale the existing matrix appropriately. Keep
        // aspect ratio if one dimension is set to 0.
